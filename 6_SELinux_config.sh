...
# Get current modes
output="$("$DIR/06_SELinux_check.sh" || true)"
...
# Verify again
if "$DIR/06_SELinux_check.sh"; then
    echo "SELinux successfully set to $TARGET_MODE (temporary and permanent)."
else
    echo "Warning: SELinux state does not fully match desired $TARGET_MODE." >&2
fi
